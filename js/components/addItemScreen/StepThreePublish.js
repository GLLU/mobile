...
class StepThreePublish extends BaseComponent {
  ...
  checkUrlOk() {
    ...
    let missingItemUrl = _.findIndex(items, function (item) {
      return !!item.url
    });
  ...
  renderBrandsUrl() {
    ...
    return _.map(items, (item, index) => {
      ...
      return <BrandUrlInput key={index} itemUrl={url} itemId={item.id} iconUrl={iconUrl}
                            shouldFocus={this.state.shouldFocus}
                            handleUrlEndEditing={(event, text) => this.handleUrlEndEditing(event, text)}/>
    });
  }
        ...
        <View key={index} style={{flexDirection: 'row'}}>
          ...
          <View style={{height: 40, padding: 2, backgroundColor: 'white', marginTop: 10, marginBottom: 10}}>
            <Image source={{uri: iconUrl}} style={[{
              height: 36, backgroundColor: 'white', borderLeftWidth: 2, width: 30,
              resizeMode: 'contain',
              alignSelf: 'center',
            }]}/>
          </View>
